...
namespace QuanLiCuaHangDienThoai
{
    public partial class frmQuanLiSanPham : Form
    {
        ...
        private void btnTImKiem_Click(object sender, EventArgs e)
        {
            ...
            string error = null;
            string giatri = tbxTimKiem.Text;
            contextTimKiemSanPham context = new contextTimKiemSanPham();
            DataSet data = new DataSet();
            if (checkID.Checked == true)
            {
                GrV.DataSource = null;
                context.Context(new TimKiemSanPhamTheoID());
                data = context.TimKiemSanPhamTheoGT(ref error, giatri);
                //data = timtheote.TimKiem(ref error, giatri);
                GrV.DataSource = data.Tables[0];
                
            }
            else
            {
                GrV.DataSource = null;
                context.Context(new TimKiemTheoTen());
                data = context.TimKiemSanPhamTheoGT(ref error, giatri);
                GrV.DataSource = data.Tables[0];
            }
